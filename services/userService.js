...
export const getUserById = async (userId) => {
	try {
		...
		const formattedEducation = user.recentEducation.map((edu) => ({
			...
			educationPassoutDate: edu.passoutYear
		...
		const formattedExperience = user.recentExperience.map((exp) => ({
			...
			companyStartDate: exp.experienceDuration.startYear
				? new Date(exp.experienceDuration.startYear).getFullYear()
				: "",
			companyEndDate: exp.experienceDuration.endYear
		...
		try {
			const pdfDataLegal = await getDocumentByUser(
				userId,
		...
		const milestone_profile = {
			...
			completion: Math.round(
				calculateProfileCompletion(user, userRequiredFields)
			),
			description:
				"Your Profile is successfully created, please complete the remaining profile",
			image:
				"https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/image+79.png",
		};

		const milestone_company = {
			name: "Company",
			completion: Math.round(
				calculateProfileCompletion(user.startUp, companyRequiredFields)
			),
			description:
				"Company Profile is successfully created, please complete the remaining details",
			image:
				"https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/image 79-1.png",
		};

		const milestone_onelink = {
			name: "OneLink",
			completion:
				user.startUp?.introductoryMessage || user.investor?.introductoryMessage
					? 100
					: 0,
			description: "Fill all details to complete Onelink profile.",
			image:
				"https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/image 79-3.png",
		};

		const milestone_documents = {
			name: "Documents",
			completion: Math.round(
				documentsCount <= 4 ? (documentsCount / 4) * 100 : 100
			),
			description:
				"Upload your business related documents to get your Onelink profile ready to share",
			image:
				"https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/image 79-2.png",
		};
		...
		const milestone_posts = {
			...
			description:
				"Hola! Create your first post to share your experience with Capital Hub.",
			image:
				"https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/image 79-1.png",
		};
		...
		if (user.startUp) {
			...
			if (user.startUp.socialLinks.website) {
				socialLinks.push({
					name: "website",
					link: user.startUp.socialLinks.website,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/website.png",
				});
			}
			if (user.startUp.socialLinks.linkedin) {
				socialLinks.push({
					name: "linkedin",
					link: user.startUp.socialLinks.linkedin,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/linkedin.png",
				});
			}
			if (user.startUp.socialLinks.instagram) {
				socialLinks.push({
					name: "instagram",
					link: user.startUp.socialLinks.instagram,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/instagram.png",
				});
			}
			if (user.startUp.socialLinks.twitter) {
				socialLinks.push({
					name: "twitter",
					link: user.startUp.socialLinks.twitter,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/twitter.png",
				});
			}
			if (user.startUp.socialLinks.facebook) {
				socialLinks.push({
					name: "facebook",
					link: user.startUp.socialLinks.facebook,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/facebook.png",
				});
			}
			if (user.startUp.contactDetails?.email) {
				socialLinks.push({
					name: "email",
					link: user.startUp.contactDetails.email,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/email.png",
				});
			}
			...
			if (user.investor.socialLinks.website) {
				socialLinks.push({
					name: "website",
					link: user.investor.socialLinks.website,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/website.png",
				});
			}
			if (user.investor.socialLinks.linkedin) {
				socialLinks.push({
					name: "linkedin",
					link: user.investor.socialLinks.linkedin,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/linkedin.png",
				});
			}
			if (user.investor.socialLinks.instagram) {
				socialLinks.push({
					name: "instagram",
					link: user.investor.socialLinks.instagram,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/instagram.png",
				});
			}
			if (user.investor.socialLinks.twitter) {
				socialLinks.push({
					name: "twitter",
					link: user.investor.socialLinks.twitter,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/twitter.png",
				});
			}
			if (user.investor.socialLinks.facebook) {
				socialLinks.push({
					name: "facebook",
					link: user.investor.socialLinks.facebook,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/facebook.png",
				});
			}
			if (user.investor.contactDetails?.email) {
				socialLinks.push({
					name: "email",
					link: user.investor.contactDetails.email,
					logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/email.png",
				});
			}
...
export const getFounderProfilePageData = async (userId, founderId) => {
	try {
		const user = await UserModel.findById(founderId).populate(
			"startUp investor"
		);
		...
		if (
			currentUser.investorIdCount.includes(founderId) ||
	...
	try {
		...
		if (currentUser.investorIdCount.length >= 5 && !currentUser.isSubscribed) {
			return {
				status: false,
				message:
					"Current user has reached the maximum number of email access. Please subscribe to access more Founders/Investors' emails.",
			};
		}
		...
		if (type === "startup") {
			...
			const curatedStartups = startups.map((startup) => {
				...
				if (startup.socialLinks.website) {
					socialLinks.push({
						name: "website",
						link: startup.socialLinks.website,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/website.png",
					});
				}
				if (startup.socialLinks.linkedin) {
					socialLinks.push({
						name: "linkedin",
						link: startup.socialLinks.linkedin,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/linkedin.png",
					});
				}
				if (startup.socialLinks.instagram) {
					socialLinks.push({
						name: "instagram",
						link: startup.socialLinks.instagram,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/instagram.png",
					});
				}
				if (startup.socialLinks.twitter) {
					socialLinks.push({
						name: "twitter",
						link: startup.socialLinks.twitter,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/twitter.png",
					});
				}
				if (startup.socialLinks.facebook) {
					socialLinks.push({
						name: "facebook",
						link: startup.socialLinks.facebook,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/facebook.png",
					});
				}
			...
			const curatedFounders = founders.map((founder) => {
				...
				const lastFunding = `${lastFundingDate.toLocaleString("default", {
					month: "long",
				})} ${lastFundingDate.getFullYear()}`;
				...
				if (startup.contactDetails?.email) {
					socialLinks.push({
						name: "email",
						link: startup.contactDetails?.email,
						logo: "https://ch-social-link-logo.s3.ap-south-1.amazonaws.com/email.png",
					});
				}
...
export const getProfilePosts = async (userId, type) => {
	try {
		const user = await UserModel.findById(userId).populate(
			"featuredPosts savedPosts companyUpdate startUp investor"
		);
		...
		const curatePost = (posts) => {
			return posts.map((post) => {
				...
				return {
					...
					userLocation: user.startUp
						? user.startUp.location
						: user.investor
						? user.investor.location
						: "",
					postId: post._id,
